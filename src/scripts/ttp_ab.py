...
class Model:
    ...
    def __init__(self, model_path=None, past_chunks=PAST_CHUNKS,
                 tcp_info=TCP_INFO, dim_out=DIM_OUT):
    ...
    def save_cpp_model(self, model_path, meta_path):
        ...
        example = torch.rand(1, self.dim_in).double()
...
def main():
    ...
    tcp_seg = ''.join([name[0] for name in TCP_INFO])
    args.save_model += '_' + tcp_seg + '_' + str(PAST_CHUNKS)
