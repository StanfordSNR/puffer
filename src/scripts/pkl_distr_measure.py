...
TTP_PATHS = {'no cwnd in_flight': \
             ...
             'single': '/ablation/0101-0115-past8-bbr',
            }
...
ABR_SETTINGS = {'no_past': ['no past no cwnd inflight',
                ...
                'tcp': ['no cwnd in_flight',
                        ...
                        'past 8',
                        'HM'],
                'simple': ['past 8', 'HM', 'single']
               }

def sq_error(est, real):
    return (est - real) * (est - real)


def rebuf_time(est, buffer_time=2):

    if not type(est) is list:
        return max(est - 2, 0)
    else:
        return sum([est[t] * (t * 0.5 - 2) for t in range(5, 21)])

def rebuf_prob(est, buffer_time=2):
    if not type(est) is list:
        return int(est > buffer_time)
    else:
        tmp = 0
        for t in range(21):
            if t * 0.5 > buffer_time:
                return (buffer_time - (t - 1) * 0.5) / 0.5 * est[t] \
                        + tmp
            tmp += est[t]
...
def trans_time(est):
    if not type(est) is list:
        ...
        return sum([est[t] * ttp_map_dis_to_real(t) for t in range(21)])
...
def collect_pred_error(d, models, args, expt_id_cache):
    ahead = args.ahead
    if args.task == 'rebuf':
        err = {setting: {i: [] for i in range(2,6)} for setting in models}
    else:
        err = {setting: {i: [] for i in range(args.ahead)} for setting in models}
    ...
    for session in d:
        ...
        for ts in d[session]:
            ...
            if ts + (ahead - 1) * VIDEO_DURATION not in d[session]:
                continue
            ...
            for setting in models:
                if setting == 'HM':
                    ...
                    est = list(ttp_pred_distr(d[session],
                                             ts, models[setting][0]))

                if args.task == 'rebuf':
                    trans_time = d[session][ts]['trans_time']
                    for i in range(2, 6):
                        real = rebuf_prob(min(10, trans_time), i)
                        est = rebuf_prob(est, i)
                        err[setting][i].append(sq_error(real, est))
                    continue


                for i in range(ahead):
                    if setting != 'HM' and setting != 'single':
                        est = list(ttp_pred_distr(d[session], ts,
                                             models[setting][i]))

                    nts = ts + i * VIDEO_DURATION
                    if args.task == 'rebuf':
                        real = rebuf_time(min(10, d[session][nts]['trans_time']))
                        est = rebuf_time(est)
                        err[setting][i].append(sq_error(real, est))
                    elif args.task == 'trans':
                        real = d[session][nts]['trans_time']
                        est = trans_time(est)
                        err[setting][i].append(sq_error(real, est))
        #        print(setting, real, est)
        #        err[setting].append(pred_error(real, est, ))
    #            print(setting, ':', real, ', ', est)
...
def get_figure(d, args):
    x = {}
    for abr in d:
        x[abr] = {'num': [len(t) for t in d[abr].values()],
                  'mean': [np.mean(t) for t in d[abr].values()],
                  'std': [np.std(t) for t in d[abr].values()],
                 }
...
def plot(expt_id_cache, args):
    ...
    pre_dp = '{}_{}_{}{}.pickle'.format(args.pre_dp, args.task, args.setting,
                                         filt)
    output = '{}_{}_{}{}.txt'.format(args.pre_dp, args.task, args.setting,
                                        filt)
    ...
    if os.path.isfile(pre_dp):
        ...
        models = {}
        models = load_models(ABR_SETTINGS[args.setting])
...
def main():
    ...
    parser.add_argument('--pre-dp', default='distr_measure_val')
    #parser.add_argument('--err-func', default='zero_one')
    parser.add_argument('--task', default='trans')
    parser.add_argument('--filt-fast', action='store_true')
    parser.add_argument('--ahead', default='5')
    parser.add_argument('--setting', default='simple')
    args = parser.parse_args()

    args.ahead = int(args.ahead)
